...
class SoMVisualizer(RotLocalVisualizer):
    ...
    def _draw_instances(self, image: np.ndarray, instances: ['InstanceData'],
        ...
        if 'bboxes' in instances:
            ...
            for i, (pos, label) in enumerate(zip(positions, labels)):
                ...
                label_text = f'{i+1} ' + label_text

                self.draw_texts(label_text,
                                pos,
    ...
    def draw_rbox_with_number(self,
                              name,
                              image,
                              data_sample,
                              show: bool = False,
        ...
        if show:
            self.show(drawn_img, win_name='', wait_time=wait_time)
...
class Visualizer:
    ...
    def draw_panoptic_seg(self,
        ...
        for mask, sinfo in pred.semantic_masks():
            ...
            text = self.metadata.stuff_classes[category_idx].replace(
                '-other', '').replace('-merged', '')
            self.draw_binary_mask(
                mask,
                color=mask_color,
                edge_color=_OFF_WHITE,
        ...
        labels = _create_text_labels(category_ids, scores, class_names,
    ...
    def draw_binary_mask_with_number(self,
        ...
        if 'Mask' in anno_mode:
            if not mask.has_holes:
                ...
                for segment in mask.polygons:
                    area = mask_util.area(
                        mask_util.frPyObjects([segment], shape2d[0],
                                              shape2d[1]))
                    if area < (area_threshold or 0):
                        continue
                    has_valid_segment = True
        ...
        if text is not None and has_valid_segment:
            ...
            self._draw_number_in_mask(binary_mask, text, lighter_color,
                                      label_mode)
        return self.output
    ...
    def _draw_number_in_mask(self, binary_mask, text, color, label_mode='1'):
        ...
        self.draw_text(text, (coords_x[len(coords_x) // 2] + 2,
