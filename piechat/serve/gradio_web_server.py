...
from piechat.utils.logger import build_logger, server_error_msg
from piechat.constants import LOGDIR
...
def http_bot(state, model_selector, temperature, top_p, max_new_tokens,
    ...
    if len(state.messages) == state.offset + 2:
        ...
        if "llava" in model_name.lower():
            if 'llama-2' in model_name.lower():
                ...
                elif 'plain' in model_name.lower(
    ...
    if worker_addr == "":
        ...
        yield (state, state.to_gradio_chatbot(), disable_btn, disable_btn,
               disable_btn, enable_btn, enable_btn)
        return
    ...
    pload = {
        ...
        float(top_p),
        "max_new_tokens":
        min(int(max_new_tokens), 1536),
        "stop":
        state.sep if state.sep_style
        in [SeparatorStyle.SINGLE, SeparatorStyle.MPT] else state.sep2,
        "images":
        f'List of {len(state.get_images())} images: {all_image_hash}',
    }
        ...
        for chunk in response.iter_lines(decode_unicode=False,
            ...
            if chunk:
                ...
                if data["error_code"] == 0:
                    ...
                    output = data[
                        "text"] + f" (error_code: {data['error_code']})"
                    state.messages[-1][-1] = output
...
def build_demo(embed_mode, cur_dir=None, concurrency_count=10):
    textbox = gr.Textbox(show_label=False,
                         ...
                         container=False)
    with gr.Blocks(title="LLaVA", theme=gr.themes.Default(),
        ...
        # Register listeners
        btn_list = [
            upvote_btn, downvote_btn, flag_btn, regenerate_btn, clear_btn
        ]
        upvote_btn.click(upvote_last_response, [state, model_selector],
                         [textbox, upvote_btn, downvote_btn, flag_btn])
        downvote_btn.click(downvote_last_response, [state, model_selector],
                           [textbox, upvote_btn, downvote_btn, flag_btn])
        flag_btn.click(flag_last_response, [state, model_selector],
                       [textbox, upvote_btn, downvote_btn, flag_btn])

        regenerate_btn.click(
            regenerate, [state, image_process_mode],
        ...
        clear_btn.click(clear_history,
                        None, [state, chatbot, textbox, imagebox] + btn_list,
                        queue=False)

        textbox.submit(
            add_text, [state, textbox, imagebox, image_process_mode],
        ...
        if args.model_list_mode == "once":
            ...
            demo.load(load_demo_refresh_model_list,
                      None, [state, model_selector],
                      queue=False)
        else:
            raise ValueError(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-list-mode",
                        type=str,
                        default="once",
                        choices=["once", "reload"])
    ...
    demo.queue(api_open=False).launch(server_name=args.host,
